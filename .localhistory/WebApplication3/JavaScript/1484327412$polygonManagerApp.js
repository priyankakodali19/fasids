...
console.log(JSON.parse($("meta[name=target-polygon]").attr("content")));
alert("asd");
        ...
        function (newStatus, oldStatus) {
            ...
            switch (newStatus) {
                ...
                case "productlisting":
                    ...
                    if (mapRelatedService.activePolygon) {
                        location.href = "/InteractiveLandscape/Treatment"; // TODO: add glblprefix to config of this application
                    }
                    break;
                case "resetting":
    ...
    $scope.saveTreatmentAndPolygonToServer = function ($event) {
        ...
        $.ajax({
            ...
            success: function (creationApiResult) {
                console.log(creationApiResult.jumpUrl)
                myfunction(geoJsonPolygon);
                x = 5;
                //$('#treatment-modal').modal('toggle');
                //location.href = "/InteractiveLandscape/Treatment";
            },
            error: function (jqXHR, textStatus, errorThrown) {
                myfunction();
