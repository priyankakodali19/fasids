...
namespace WebApplication3.Models.EntityManager
{
    public class InteractiveLandscapeManager
    {
        ...
        public ProductTreatmentInfo GetProductTreatmentInfo(string polygon_id)
        {
            ...
            using (FASIDSEntities4 db = new FASIDSEntities4())
            {
                ...
                var polygon = db.PolygonGeoJsons.Where(o => o.GeoJsonId.Equals(polygon_id));
                if (polygon.Any())
                {
                    Boolean IsOutdoor = Convert.ToBoolean(polygon.FirstOrDefault().IsOutdoor);
                    Boolean NeedOrganic = Convert.ToBoolean(polygon.FirstOrDefault().NeedOrganic);
                    Boolean NeedSafeForPets = Convert.ToBoolean(polygon.FirstOrDefault().NeedSafeForPets);
                    ...
                    {
                        command.CommandText = "select * from ProductData where ProductControlMethod=@ProductControlMethod and ProductUsage=@ProductUsage ";
                        command.Parameters.AddWithValue("@ProductControlMethod", ControlMethod);
                        command.Parameters.AddWithValue("@ProductUsage", Usage);
