        ...
        public Boolean AddProductDetails3(String ProductDetails)
        {
            try
            {
                using (FASIDSEntities db = new FASIDSEntities())
                {
                    ...
                    foreach (TreatmentSiteCode obj in db.TreatmentSiteCodes)
                    {
                        for (int y = 0; y < dynObj.UrbanTreatmentSitesBroadcast.Count; y++)
                        {
                            if (dynObj.UrbanTreatmentSitesBroadcast[y].optionText.trim() == obj.SiteType)
                            {
                                PL.UrbanBroadcastSites = obj.Code;
                            }
                        }

                        for (int y = 0; y < dynObj.UrbanTreatmentSitesMound.Count; y++)
                        {
                            if (dynObj.UrbanTreatmentSitesMound[y].optionText.trim() == obj.SiteType)
                            {
                                PL.UrbanMoundSites = obj.Code;
                            }
                        }

                        for (int y = 0; y < dynObj.AgriTreatmentSitesBroadcast.Count; y++)
                        {
                            if (dynObj.AgriTreatmentSitesBroadcast[y].optionText.trim() == obj.SiteType)
                            {
                                PL.AgriBroadcastSites = obj.Code;
                            }
                        }

                        for (int y = 0; y < dynObj.AgriTreatmentSitesMound.Count; y++)
                        {
                            if (dynObj.AgriTreatmentSitesMound[y].optionText.trim() == obj.SiteType)
                            {
                                PL.AgriTreatmentSitesMound = obj.Code;
                            }
                        }
