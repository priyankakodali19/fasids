...
namespace WebApplication3.Models.EntityManager
{
    public class UserManager
    {
        ...
        public bool IsEmailIdExist(string email)
        {
            using (FASIDSEntities db = new FASIDSEntities())
            {
                return db.UserDatas.Where(o => o.EmailId.Equals(email)).Any();
            }
        }
        ...
        public string GetUserPasswordEmail(string email)
        {
            using (FASIDSEntities db = new FASIDSEntities())
            {
                var user = db.UserDatas.Where(o => o.EmailId.Equals(email));
                if (user.Any())
                    return user.FirstOrDefault().Password;
                else
                    return string.Empty;
