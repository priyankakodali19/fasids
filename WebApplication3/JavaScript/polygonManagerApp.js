...
polygonManagerApp.controller("pmaModalsCtrl", function ($scope, pmaConstants, stateService, mapRelatedService, mapRelatedFunctionsService) {
    ...
    var $treatmentModal = $("#treatment2-modal");
    ...
    { value: "Home Lawns / Ornamentals", optionText: "Home Lawns / Ornamentals" },
    { value: "Perimeter", optionText: "Perimeter" },
    { value: "Recreational Areas", optionText: "Recreational Areas" },
    { value: "Right of Way", optionText: "Right of Way" }
    ];

    $scope.TypeOfAgricultureTreatmentSite = [
   { value: "Pasture Land", optionText: "Pasture Land" },
   { value: "Poultry Houses", optionText: "Poultry Houses" },
   { value: "Crop Areas", optionText: "Crop Areas" },
   { value: "Sod/Turf", optionText: "Sod / Turf" },
   { value: "Nursery Container", optionText: "Nursery Container" },
   { value: "Nursery B & B (Ball & Burlap)", optionText: "Nursery B & B (Ball & Burlap)" },
   { value: "Other ( Fences / Rows / Ditches / Out Buildings)", optionText: "Other ( Fences / Rows / Ditches / Out Buildings)" }
    ];
    ...
    $scope.optionsForUsage = [
     { value: "broadcast", optionText: "Broadcast Treatment", explaination: "You want to focus on several fire ant mounds" },
     { value: "imt", optionText: "Individual Mound Treatment", explaination: "It is done mainly for preventive purpose" },
     { value: "other", optionText: "Product that Can Do Both", explaination: "Product that can do both." }
    ];
    ...
    $scope.$watch(
      function ($scope) {
          ...
          //-------treatment2------------//
          return $scope.treatment2.type_of_treatment;
      },
      function (newValue, oldValue) {
          if (oldValue === "imt") {
              ...
              //-------treatment2------------//
              return $scope.treatment2.mound_number;
          }
      }
    );
    ...
    $scope.mergeTreatmentAndRender = function () {
        ...
        //-------treatment2------------//
        angular.extend(activePolygon.properties, $scope.treatment2);
    ...
    $scope.saveTreatmentAndPolygonToServer = function ($event) {
        ...
        //-------treatment2------------//
        angular.extend(geoJsonPolygon.properties, $scope.treatment2);
    ...
    $scope.fillTreatmentForm = function (googleMVCObjectPolygon) {  //TODO
        ...
        //---------------------treatment2-------------------------//
        angular.forEach($scope.treatment2, function (value, keyName) {
            if (angular.isDefined(googleMVCObjectPolygon.properties[keyName])) {
                $scope.treatment2[keyName] = angular.copy(googleMVCObjectPolygon.properties[keyName]);
            }
        });
